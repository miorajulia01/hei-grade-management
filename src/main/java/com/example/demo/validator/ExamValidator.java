...
public final class ExamValidator {
  ...
  public static void validate(Exam exam) {
    ...
    if (exam.getCourse() == null) {
      throw new IllegalArgumentException("Exam must be associated with a course");
    }
    ...
    if (exam.getOrder() == null || exam.getOrder() < 1) {
      throw new IllegalArgumentException("Exam order must be greater than or equal to 1");
    }
  }

  public static void validateCoefficientsSum(double totalCoefficient) {
    if (Math.abs(totalCoefficient - 1.0) > 0.000001) {
      throw new IllegalArgumentException("The sum of exam coefficients must be equal to 1");
    }
  }
}
