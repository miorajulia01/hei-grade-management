...
class DocExportServiceTest {
    ...
    @Mock
    private DocExportRepository docExportRepository;
    ...
    void shouldGetAllDocExports() {
        when(docExportRepository.findAll())
                .thenReturn(List.of(docExportEntity));

        List<DocExport> result =
                docExportService.getAllDocExports();
    ...
    void shouldGetDocExportById() {
        when(docExportRepository.findById("export-1"))
                .thenReturn(Optional.of(docExportEntity));

        DocExport result =
                docExportService.getDocExportById("export-1");
    ...
    void shouldThrowExceptionWhenDocExportNotFound() {
        when(docExportRepository.findById("unknown"))
                .thenReturn(Optional.empty());

        RuntimeException exception =
                assertThrows(
                        RuntimeException.class,
                        () -> docExportService.getDocExportById("unknown"));

        assertEquals(
                "DocExport not found with id: unknown",
    ...
    void shouldSaveDocExportWithUser() {
        ...
        when(docExportRepository.save(any(JDocExport.class)))
                .thenReturn(docExportEntity);

        DocExport result =
                docExportService.saveDocExport(docExportModel);
        ...
        ArgumentCaptor<JDocExport> captor =
                ArgumentCaptor.forClass(JDocExport.class);
    ...
    void shouldSaveDocExportWithoutUser() {
        ...
        DocExport result =
                docExportService.saveDocExport(model);
