...
class GroupAssignmentServiceTest {
    ...
    void shouldGetAllGroupAssignments() {
        when(groupAssignmentRepository.findAll())
                .thenReturn(List.of(assignmentEntity));

        List<GroupAssignment> result =
                groupAssignmentService.getAllGroupAssignments();
    ...
    void shouldGetGroupAssignmentById() {
        ...
        GroupAssignment result =
                groupAssignmentService.getGroupAssignmentById("assignment-1");
    ...
    void shouldThrowExceptionWhenGroupAssignmentNotFound() {
        when(groupAssignmentRepository.findById("unknown"))
        ...
        RuntimeException exception =
                assertThrows(
                        ...
                        () ->
                                groupAssignmentService.getGroupAssignmentById(
                                        "unknown"));

        assertEquals(
                "GroupAssignment not found with id: unknown",
    ...
    void shouldSaveGroupAssignmentWithoutRelations() {
        when(groupAssignmentRepository.save(any(JGroupAssignment.class)))
                .thenReturn(assignmentEntity);

        GroupAssignment result =
                groupAssignmentService.saveGroupAssignment(assignmentModel);

        assertNotNull(result);
        assertEquals("assignment-1", result.getId());

        ArgumentCaptor<JGroupAssignment> captor =
                ArgumentCaptor.forClass(JGroupAssignment.class);
                ...
                GroupAssignment.builder()
                        ...
                        .student(
                                com.example.demo.model.Student.builder()
                                        ...
                                        .build())
                        .group(
                                com.example.demo.model.Group.builder()
                                        .id("group-1")
                                        .build())
                        .semester(
