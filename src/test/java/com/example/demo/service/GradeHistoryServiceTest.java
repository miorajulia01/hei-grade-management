...
class GradeHistoryServiceTest {
  ...
  void shouldGetAllGradeHistories() {
    ...
    List<GradeHistory> result = gradeHistoryService.getAllGradeHistories();
    ...
    assertEquals("Correction de la note", result.get(0).getReason());
  ...
  void shouldGetGradeHistoryById() {
    when(gradeHistoryRepository.findById("history-1")).thenReturn(Optional.of(historyEntity));

    GradeHistory result = gradeHistoryService.getGradeHistoryById("history-1");
  ...
  void shouldThrowExceptionWhenGradeHistoryNotFound() {
    when(gradeHistoryRepository.findById("unknown")).thenReturn(Optional.empty());

    RuntimeException exception =
        assertThrows(
            RuntimeException.class, () -> gradeHistoryService.getGradeHistoryById("unknown"));

    assertEquals("GradeHistory not found with id: unknown", exception.getMessage());
  ...
  void shouldSaveGradeHistoryWithoutRelations() {
    ...
    GradeHistory result = gradeHistoryService.saveGradeHistory(historyModel);

    assertNotNull(result);
    assertEquals("history-1", result.getId());
    assertEquals(10.0, result.getOldScore());
    assertEquals(15.0, result.getNewScore());
    assertEquals("Correction de la note", result.getReason());
    ...
    assertEquals("Correction de la note", savedEntity.getReason());
  ...
  void shouldSaveGradeHistoryWithGrade() {
    JGrade grade = JGrade.builder().id("grade-1").score(10.0).build();
    ...
    GradeHistory model =
        GradeHistory.builder()
            ...
            .newScore(15.0)
            .reason("Correction de la note")
            .grade(com.example.demo.model.Grade.builder().id("grade-1").score(10.0).build())
            .build();

    when(gradeRepository.findById("grade-1")).thenReturn(Optional.of(grade));
    ...
    assertEquals(grade, captor.getValue().getGrade());
  ...
  void shouldThrowExceptionWhenGradeNotFound() {
    GradeHistory model =
        GradeHistory.builder()
            ...
            .newScore(15.0)
            .reason("Correction de la note")
            .grade(com.example.demo.model.Grade.builder().id("unknown").score(10.0).build())
            .build();

    when(gradeRepository.findById("unknown")).thenReturn(Optional.empty());

    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> gradeHistoryService.saveGradeHistory(model));
  ...
  void shouldRejectHistoryWithoutReason() {
    GradeHistory model =
        GradeHistory.builder().id("history-1").oldScore(10.0).newScore(15.0).reason("").build();
