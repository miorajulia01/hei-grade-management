...
class GradeServiceTest {
  ...
  void shouldGetGradeById() {
    when(gradeRepository.findById("grade-1")).thenReturn(Optional.of(gradeEntity));
  ...
  void shouldThrowExceptionWhenGradeNotFound() {
    ...
    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> gradeService.getGradeById("unknown"));
  ...
  void shouldThrowExceptionWhenStudentNotFound() {
    ...
    Grade model =
        Grade.builder()
            .id("grade-1")
            .student(Student.builder().id("unknown").build())
            .exam(examModel)
            .score(15.0)
            .weightedScore(7.5)
            .build();
    ...
    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> gradeService.saveGrade(model));
  ...
  void shouldThrowExceptionWhenExamNotFound() {
    ...
    Grade model =
        Grade.builder()
            .id("grade-1")
            .student(studentModel)
            .exam(Exam.builder().id("unknown").build())
            .score(15.0)
            .weightedScore(7.5)
            .build();
  ...
  void shouldRejectGradeWithoutStudent() {
    Grade model =
        Grade.builder().id("grade-1").exam(examModel).score(15.0).weightedScore(7.5).build();

    IllegalArgumentException exception =
        assertThrows(IllegalArgumentException.class, () -> gradeService.saveGrade(model));

    assertEquals("Grade must be associated with a student", exception.getMessage());
  ...
  void shouldRejectGradeWithoutExam() {
    Grade model =
        Grade.builder().id("grade-1").student(studentModel).score(15.0).weightedScore(7.5).build();

    IllegalArgumentException exception =
        assertThrows(IllegalArgumentException.class, () -> gradeService.saveGrade(model));

    assertEquals("Grade must be associated with an exam", exception.getMessage());
  ...
  void shouldRejectInvalidScore() {
    ...
    assertEquals("Score must be between 0 and 20", exception.getMessage());
  ...
  void shouldCalculateWeightedAverage() {
    JExam exam1 = JExam.builder().id("exam-1").coefficient(0.4).build();

    JExam exam2 = JExam.builder().id("exam-2").coefficient(0.6).build();

    JGrade grade1 = JGrade.builder().id("grade-1").score(10.0).exam(exam1).build();

    JGrade grade2 = JGrade.builder().id("grade-2").score(16.0).exam(exam2).build();

    when(gradeRepository.findByStudentId("student-1")).thenReturn(List.of(grade1, grade2));

    Double result = gradeService.calculateWeightedAverage("student-1");
  ...
  void shouldCalculateZeroWhenNoCoefficients() {
    JGrade grade = JGrade.builder().id("grade-1").score(15.0).exam(null).build();

    when(gradeRepository.findByStudentId("student-1")).thenReturn(List.of(grade));
  ...
  void shouldCalculateAverageIgnoringGradeWithoutExam() {
    JExam exam = JExam.builder().id("exam-1").coefficient(0.5).build();

    JGrade gradeWithExam = JGrade.builder().id("grade-1").score(14.0).exam(exam).build();

    JGrade gradeWithoutExam = JGrade.builder().id("grade-2").score(20.0).exam(null).build();
  ...
  void shouldGetRetakeGradesForStudent() {
    JGrade normalGrade = JGrade.builder().id("grade-1").score(15.0).build();

    JGrade retakeGrade = JGrade.builder().id("grade-2").score(5.0).build();

    when(gradeRepository.findByStudentId("student-1"))
        .thenReturn(List.of(normalGrade, retakeGrade));

    List<Grade> result = gradeService.getRetakeGradesForStudent("student-1");
  ...
  void shouldReturnEmptyListWhenStudentHasNoRetake() {
    JGrade grade = JGrade.builder().id("grade-1").score(15.0).build();
